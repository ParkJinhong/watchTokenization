...
📄 **기획·리스크 분석 → [문서(HTML)](docs/시계토큰화_기획_리스크.html)** · 🗺️ **시스템 구조도 → [전체 구조도](docs/구조도.html)** · 시장별: [A. 토큰화](docs/구조도_토큰화시장.html) · [B. 대출](docs/구조도_대출시장.html) (브라우저에서 열기 / Ctrl+P로 PDF 저장)

🖥️ **토큰화 시장 웹 테스트 도구 → `frontend/`** — 토큰 발행(3가지) · P2P 거래 · 대여수수료 자동분배 · 배당 출금을 브라우저에서 (아래 [실행](#토큰화-시장-웹-테스트-도구) 참고)
...
대출(B)을 제외한 **토큰화 시장(A)**을 브라우저에서 직접 굴려보는 도구입니다. MetaMask 없이 Hardhat 기본 계정을 골라가며 테스트합니다.

```bash
# 터미널 A — 로컬 체인
npx hardhat node

# 터미널 B — 토큰화 앱 컨트랙트 배포 (frontend/contracts.js 생성)
npm run deploy:app
...
1. **토큰 발행** — ① 회사 보유분 ② 자금 모집 분배 ③ 토큰화 희망자 중개 (세 가지 방법)
2. **P2P 거래** — 매도/매수 주문 등록·체결(부분 체결)·취소
3. **대여 수수료** — 회사가 10초마다 자동 분배(또는 1회 분배)
4. **배당 출금** — 보유자가 자기 몫을 청구. 거래로 토큰이 이동해도 배당 지분이 정확히 따라감

> 컨트랙트: `WatchShare`(배당형 ERC-20) · `WatchFactory`(3가지 발행) · `P2PMarket`(주문 거래). 결제 통화는 `MockUSDC`.
